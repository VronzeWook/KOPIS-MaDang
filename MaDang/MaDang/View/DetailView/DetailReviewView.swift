...
struct DetailReviewView: View {
    ...
    @Binding var isDeleteModalPresented: Bool
    @Binding var isChangedReview: Bool
    ...
    var body: some View {
        ...
        .onChange(of: isChangedReview) { _ in
            FirestoreManager.shared.fetchReviewsByPerformance(performanceId: perform.id) { result in
                switch result {
                case .success(let reviews):
                    if reviews.isEmpty {
        ...
        .sheet(isPresented: $showModal) {
            if let review = selectedReview {
                ReviewActionSheet(isPresented: $showModal, review: review, isReportModalPresented: $isReportModalPresented, isDeleteModalPresented: $isDeleteModalPresented)
